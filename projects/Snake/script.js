...
function setup() {
  ...
  const img = loadImage("snake.png", (image) => {
    
    for(let i = 0; i < 4; i++) {
      snakeTextures[i] = image.get((i%2)*32, floor(i/2)*32, 32, 32)
  ...
  for (let x = 0; x < grid_w; x++) {
    for (let y = 0; y < grid_h; y++) {
      if (grid[x][y] > 0) {
        ...
        image(snakeTextures[1], x*cellSize, y*cellSize, cellSize, cellSize)
      }
      else if (grid[x][y] < 0) {
        //Draw food
        fill(100, 100, 50);
