...
function setup() {
  ...
  cell_h = floor(height / f_height) * 0.9;
...
function draw() {
  ...
  fill(255);
  text("Bombs: "+bombcount+" | remaining cells: "+(CellsToDiscover - bombcount), 20, 20);
  ...
  rect(width - 100, 5, 90, 40, 5);
  fill(255);
  text("FLAGMODE", width - 95, 30);
  ...
  if (touches.length > 0 && clickcounter > 10) {
    clickcounter = 0;
    //ceck if touch is in flagmode field
    if(touches[0].x > width - 100 && touches[0].x < width && touches[0].y < 40){
      flagmode = !flagmode;
    }
    else{
      //calculate pressed cell

      var pressed_cell_x = floor((touches[0].x)/ cell_w) ;
      var pressed_cell_y = floor((touches[0].y - y_offset) / cell_h);
      console.log(pressed_cell_x, pressed_cell_y);
      ...
      else
      {
        ...
        if (!flagmode && !field[pressed_cell_x][pressed_cell_y].flag)
        {
        ...
        else if (flagmode && !field[pressed_cell_x][pressed_cell_y].discovered)
        {
