...
function getImgPixel(img, x, y) : number[] {
...
function getArrayPixel(array, x, y) : number[] {
    x = Math.max(0, Math.min(width-1, x))
    y = Math.max(0, Math.min(height-1, y))
    let c = []
...
function imgToArray(img : p5.Image) : number[] {
    const res = []
...
function ArrayToImg(array : number[]) : p5.Image {
...
function generateKernel(size) : Coords2D[] {
    const kernel = []
...
function multiply(img1 : p5.Image, img2: p5.Image) : p5.Image {
...
function edgeDetection(img : p5.Image) : p5.Image {
...
function blurOverlay(img: p5.Image) : p5.Image {
