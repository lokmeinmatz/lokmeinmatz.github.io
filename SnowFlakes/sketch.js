...
function newPopulation() {
  ...
  cars = []
  console.log("Randomizing by " + (50 / (best.distTravelled + 0.1)).toString())
  for(let i = 0; i < popSize/2; i++) {
    cars[i] = best.copy()
    cars[i].reset(startVavg)
    cars[i].brain.randomize(50 / (best.distTravelled + 0.2))
  }
  while(cars.length < popSize - 2) {
    let ncar = second.copy()
    ncar.reset(startVavg)
    ncar.brain.randomize(100 / (best.distTravelled + 0.2))
    cars.push(ncar)
  }
  cars.push(best.copy())
